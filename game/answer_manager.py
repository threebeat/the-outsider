...
class AnswerData:
    ...
    is_outsider_answering: bool = False  # Track if outsider is answering
...
class AnswerManager:
    ...
    def create_answer_data(self, 
                          ...
                          ai_generated: bool = False,
                          is_outsider_answering: bool = False) -> Optional[AnswerData]:
        """
        ...
        Args:
            ...
            is_outsider_answering: Whether the outsider AI is answering
        ...
        try:
            ...
            answer_data = AnswerData(
                ...
                is_outsider_answering=is_outsider_answering
            )
            
            logger.debug(f"Created answer data: {answerer} (outsider={is_outsider_answering})")
            return answer_data
    ...
    def generate_ai_outsider_answer(self, 
                                  answerer: str,
                                  question: str,
                                  asker: str,
                                  game_context: Dict[str, Any]) -> Optional[str]:
        """
        Generate an answer from the AI outsider who doesn't know the location.
        
        Args:
            answerer: AI outsider answering the question
        ...
        try:
            ...
            logger.info(f"Generated AI outsider answer: {answerer}")
            return answer
            
        except ImportError:
            logger.warning("AI system not available for outsider answer generation")
            return self._get_fallback_outsider_answer(question)
            
        except Exception as e:
            logger.error(f"Error generating AI outsider answer: {e}")
            return self._get_fallback_outsider_answer(question)
    
    def generate_ai_regular_answer(self, 
                                 answerer: str,
                                 question: str,
                                 asker: str,
                                 game_context: Dict[str, Any]) -> Optional[str]:
        """
        Generate an answer from a regular AI player who knows the location.
        
        Args:
            answerer: Regular AI player answering the question
            question: The question being answered
            asker: Username of who asked the question
            game_context: Game context (location, conversation history, etc.)
            
        Returns:
            Generated answer string or None if failed
        """
        try:
            # Regular AI players know the location
            from ai import AnswerGenerator
            ...
            logger.info(f"Generated regular AI answer: {answerer}")
            return answer
            
        except ImportError:
            logger.warning("AI system not available for regular AI answer generation")
            return self._get_fallback_regular_answer(question)
            
        except Exception as e:
            logger.error(f"Error generating regular AI answer: {e}")
            return self._get_fallback_regular_answer(question)
    ...
    def handle_human_answer(self, 
        ...
        try:
            ...
            answer_data = self.create_answer_data(
                ...
                ai_generated=False,
                is_outsider_answering=False  # Humans are never the outsider
            )
    ...
    def handle_ai_answer(self,
                       ...
                       question_id: Optional[str] = None) -> Optional[AnswerData]:
        """
        Handle an answer from an AI player (could be outsider or regular).
        ...
        try:
            if is_outsider:
                # Generate outsider answer (doesn't know location, gives vague answers)
                answer = self.generate_ai_outsider_answer(answerer, question, asker, game_context)
            else:
                # Generate regular AI answer (knows location, gives specific answers)
                answer = self.generate_ai_regular_answer(answerer, question, asker, game_context)
            ...
            answer_data = self.create_answer_data(
                ...
                question_id=question_id,
                ai_generated=True,
                is_outsider_answering=is_outsider
            )
            
            if answer_data:
                logger.info(f"AI answer processed: {answerer} (outsider={is_outsider})")
    ...
    def _get_fallback_outsider_answer(self, question: str) -> str:
        """Get a fallback answer for the outsider when AI generation fails."""
        import random
        
        question_lower = question.lower()
        
        # Outsider answers are vague but confident, could apply anywhere
    ...
    def _get_fallback_regular_answer(self, question: str) -> str:
        """Get a fallback answer for regular AI when AI generation fails."""
        import random
        
        # Regular AI answers are more specific (but still generic since no location context)
        fallback_answers = [
            ...
            "Based on what I know, that's how things typically work."
        ]
    ...
    def format_answer_for_broadcast(self, answer_data: AnswerData) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                'is_outsider_answering': answer_data.is_outsider_answering
            }
    ...
    def analyze_outsider_answer_quality(self, 
                                       answer_data: AnswerData,
                                       question: str,
                                       known_location: str) -> Dict[str, Any]:
        """
        Analyze how well an outsider answer conceals their lack of knowledge.
        ...
        """
        try:
            if not answer_data.is_outsider_answering:
                return {'error': 'Not an outsider answer'}
            ...
            logger.error(f"Error analyzing outsider answer quality: {e}")
            return {'error': str(e)}
    ...
    def get_answer_statistics(self, answers: List[AnswerData]) -> Dict[str, Any]:
        ...
        try:
            ...
            outsider_answers = sum(1 for a in answers if a.is_outsider_answering)
            ...
            return {
                ...
                'outsider_answers': outsider_answers,
                'regular_ai_answers': ai_answers - outsider_answers,
